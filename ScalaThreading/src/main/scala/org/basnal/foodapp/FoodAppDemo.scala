...
object FoodAppDemo extends IOApp.Simple {
  ...
  private def printTable(label: String): IO[Unit] = {
    for {
      itemTbl <- Database.fetchItemTable
      areaTbl <- Database.fetchAreaTable
      restTbl <- Database.fetchRestaurantTable
      _ <- IO.println(s"\n--- $label ---")
      
      // Print area status
      _ <- IO.println("\nAreas:")
      _ <- IO.pure(areaTbl.ids.indices.map { i =>
        val idFrag = areaTbl.ids(i).value.toString.take(4)
        s"area $idFrag | isOpen = ${areaTbl.isOpen(i)}"
      }.mkString("\n")).flatMap(IO.println)
      
      // Print restaurant status
      _ <- IO.println("\nRestaurants:")
      _ <- IO.pure(restTbl.ids.indices.map { i =>
        val idFrag = restTbl.ids(i).value.toString.take(4)
        val areaFrag = areaTbl.ids(restTbl.areaIdx(i)).value.toString.take(4)
        s"rest $idFrag | area $areaFrag | isOpen = ${restTbl.isOpen(i)}"
      }.mkString("\n")).flatMap(IO.println)
      
      // Print items
      _ <- IO.println("\nItems:")
      _ <- IO.pure(itemTbl.ids.indices.map { i =>
        val idFrag = itemTbl.ids(i).value.toString.take(4)
        val restFrag = restTbl.ids(itemTbl.restaurantIdx(i)).value.toString.take(4)
        val areaFrag = areaTbl.ids(itemTbl.areaIdx(i)).value.toString.take(4)
        s"item $idFrag | rest $restFrag | area $areaFrag | inStock = ${itemTbl.inStock(i)}"
      }.mkString("\n")).flatMap(IO.println)
  ...
  override def run: IO[Unit] = for {
    _ <- (IO.println("Seeding initial data") *>
          (Database.saveAreaTable(areaTable)
            *> Database.saveRestaurantTable(restaurantTable)
            *> Database.saveItemTable(initialItemTable)))
